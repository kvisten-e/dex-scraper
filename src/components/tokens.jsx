...
export default function Tokens({ days, triggerAction }) {
  ...
  //       const fetchTokens = await queryLpsBetween(
  //         unixTimestamp - convertDaysToSeconds(days),
  //         unixTimestamp
  //       );
  //       console.log("FetchedTokens: ", fetchTokens);
  ...
  //               } else if (lpStatus[lpStatus.length - 2].type === "WITHDRAW_LIQUIDITY") {
  ...
  //             tokenObject.pool_created_at = poolData.data.attributes.pool_created_at
  ...
  //       console.log("Response supply: ", response.value.uiAmountString);
        ...
        <tbody>
          {result.map((token, index) => (
            <tr key={index}>
              ...
              <td>{roundValue(token.startValueLp, 4)}</td>
