...
export default function PresentResult(props) {
  ...
  const { process, setProcess, switchButton, setSwitchButton } =
            ...
            async function main() {
              ...
              async function fetchMainWalletTransactions() {
                ...
                try {
                  let signatures = [];
                  if (wallet.length === 1) {
                    signatures = await rotateRPC().getSignaturesForAddress(
                      getPublickey(wallet[0]),
                      {
                        limit: Number(params[0].total_tx),
                        commitment: "finalized",
                      }
                    );

                    setProcess((prevProcess) =>
                      prevProcess.map((step, index) => ({
                        ...step,
                    ...
                    for (let eachWallet of wallet) {
                      let signatureFetch =
                        await rotateRPC().getSignaturesForAddress(
                          getPublickey(eachWallet),
                          {
                            limit: Number(params[0].total_tx),
                            commitment: "finalized",
                          }
                      ...
                      setProcess((prevProcess) =>
                        prevProcess.map((step, index) => ({
                          ...step,
                          completed:
                            index === 0 ? statusCompleted : step.completed,
                        }))
                      );
                    }
                  }
                  ...
                  if (signatures.length > 0) {
                    ...
                    try {
                      listTransactions = signatures.map(
                    ...
                    const confirmedTransactions =
                      await checkSolAmountTransaction(
                        wallet,
                        listTransactions,
                        Number(params[1].min_tx_value),
                        Number(params[2].max_tx_value)
                      );
                    console.log(
                    ...
                    let count = 1;
                    const transactionPromises = confirmedTransactions.map(
                      (obj, index) =>
                        delay(index * 200).then(async () => {
                          const checkForTransactions =
                            await findTransactionsFromWallet(
                              obj.wallet,
                              params[3].min_eq_tx,
                              params[5].min_eq_value_tx,
                              params[6].total_min_tx
                            );
                          let statusCompleted =
                            (count++ / confirmedTransactions.length) * 100;
                          setProcess((prevProcess) =>
                            prevProcess.map((step, idx) => ({
                              ...step,
                              completed:
                                idx === 2 ? statusCompleted : step.completed,
                            }))
                          );

                          const wallets = checkForTransactions.map(
                            (transaction) => transaction.wallets
                          );
                          if (
                            wallets.length > 0 &&
                            wallets.length === new Set(wallets).size
                          ) {
                            return {
                              wallet: obj.wallet,
                              amount: obj.amount,
                              ...
                              walletSentOut: checkForTransactions,
                            };
                          }
                          return null;
                        })
                    ...
                    filteredResults = allTransactionsResults.filter(
                      (result) => result !== null
                    );
                  }
                } catch (error) {
              ...
              async function getWalletTransactions(
                ...
                while (!success && attempts <= 4) {
                  try {
                    ...
                    const filterGroupedData = groupedData.filter(
                      (obj) =>
                        obj.wallets.length >= min_eq_tx &&
                        obj.amount >= min_eq_value_tx
                    );
                    success = true;
                    ...
                    notify(error.message);
                    console.log(
                      "Fetch failed on wallet:",
                      wallet,
                      ".. trying again on attempt: ",
                      attempts
                    );
                    attempts++;
                  }
              ...
              async function checkSolAmountTransaction(
                wallet,
                list,
                min_amount,
                max_amount
              ...
              async function findTransactionsFromWallet(
                wallet,
                min_eq_tx,
                min_eq_value_tx,
                total_min_tx
                ...
                while (retriesFirst > 0) {
                  try {
                    const signatures =
                      await rotateRPC().getSignaturesForAddress(
                        publicKeySearch,
                        { limit: Number(total_min_tx) }
                      );

                    if (signatures.length > 0) {
                      const listTransactions = signatures.map(
                        (signature) => signature.signature
                      );
                      let transactions = [];
                      for (let signature of listTransactions) {
                        ...
                        while (!success && retries > 0) {
                          try {
                            transactionDetails =
                              await rotateRPC().getParsedTransaction(
                                signature,
                                {
                                  commitment: "finalized",
                                  maxSupportedTransactionVersion: 0,
                        ...
                        if (success && transactionDetails) {
                          for (const instruction of transactionDetails
                      ...
                      let filteredAmountToWallets = Object.keys(
                        amountToWallets
                      ).reduce((acc, amount) => {
                        if (
                          amountToWallets[amount].length >= Number(min_eq_tx)
                        ) {
                          acc[amount] = amountToWallets[amount];
                        }
                        return acc;
                      }, {});
                      const findTransactionsFromWallet = Object.entries(
                        filteredAmountToWallets
                      ).map(([amount, wallets]) => ({
                      ...
                      const filterOutAllEqualWallets =
                        findTransactionsFromWallet.filter(
                          (obj) => !allEqual(obj.wallets)
                        );
  ...
  useEffect(() => {
    ...
    if (ready.length === 3 && ready !== null) {
      const sortedDataByPrice = finishedResult.sort((p1, p2) =>
            ...
            sortedData.map((obj, index) => (
              <section>
                ...
                <h4>
                  ...
                  >
                    {" "}
                    {obj.wallet.substring(0, 4)}...
                    {obj.wallet.substring(obj.wallet.length - 4)} received:{" "}
                    {obj.amount} sol
                  </a>
                </h4>
                <ul>
                  {obj.walletSentOut.map((foundWallets) => (
                    <div>
                      <p>Dex: {obj.dex}</p>
                      <p>SOL amount: {foundWallets.amount} </p>
                      <ul>
                        {foundWallets.wallets.map((eachWallet) => (
                          <li>
                            <a
                              href={
                                "https://solscan.io/account/" +
